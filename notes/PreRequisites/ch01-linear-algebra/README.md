...
2. **A bag of features.** The vector $[\text{MedInc},\,\text{HouseAge},\,\text{AveRooms},\ldots]$ describes a California district.
...
x = np.array([3.5, 32.0, 5.4])                  # three features
W = np.array([0.6, -0.1, 0.05])                 # one weight per feature
bias = -1.2
y_hat = W @ x + bias                            # 0.6*3.5 - 0.1*32 + 0.05*5.4 - 1.2
...
- **Conflating units.** If `MedInc` is in $10k and `MedHouseVal` is in $100k, the fitted $w$ carries a unit: "$100k per $10k of income". Plot with labels or you will misread the scale.
- **No bias term.** Dropping $b$ forces the line through the origin — usually wrong. A zero-income district does not have a zero house value; $b$ carries the baseline.
