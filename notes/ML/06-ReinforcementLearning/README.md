...
| **#2** | **EFFICIENCY** | Learn with minimal episodes | Real-world interactions are expensive. A robot can't fall 10,000 times to learn to walk |
| **#3** | **SCALABILITY** | Handle large/continuous state spaces | GridWorld has 16 states. Atari has $10^9$. Robotics has continuous dimensions |
| **#4** | **STABILITY** | Converge reliably | RL is notoriously unstable. Small hyperparameter changes can cause catastrophic divergence |
| **#5** | **GENERALIZATION** | Transfer to new environments | An agent trained in one maze should generalize, not memorize a single layout |
...
|----|-------|-------------|-------------|-------------|
| **1** | [Markov Decision Processes](ch01-mdps/) | Formal RL framework — states, actions, rewards, policies | Foundation | Bellman equations |
| **2** | [Dynamic Programming](ch02-dynamic-programming/) | Optimal policy when model is known | #1 Optimality ✅ | Value & policy iteration |
| **3** | [Q-Learning & TD Learning](ch03-q-learning/) | Learn without knowing transition probabilities | #1 ✅ #2 Partial | Temporal difference, ε-greedy |
| **4** | [Deep Q-Networks](ch04-dqn/) | Scale to large state spaces (Atari, CartPole) | #1 ✅ #3 ✅ | Experience replay, target networks |
| **5** | [Policy Gradients](ch05-policy-gradients/) | Handle continuous actions, direct policy optimization | #1 ✅ #3 ✅ #4 Partial | REINFORCE, actor-critic |
| **6** | [Modern RL](ch06-modern-rl/) | State-of-the-art stability & efficiency | **All 5 ✅** | PPO, SAC, A3C |
...
**Status**: **All 5 constraints addressed.** Modern RL algorithms balance optimality, efficiency, scalability, stability, and generalization.
...
This track is **theory-only** — README.md files with mathematical derivations, pseudocode, Mermaid diagrams, and ASCII art. No Jupyter notebooks. The focus is on understanding algorithms conceptually before implementing them, which prepares you for hands-on RL libraries (Stable Baselines3, CleanRL) in future projects.
