...
For SmartVal AI's California Housing dataset (~20,000 rows), a well-tuned gradient boosting model is a legitimate competitor. We use NNs here because the track is about learning them — not because they are automatically the best tool.
...
**Next up:** [Ch.2 — Neural Networks](../ch02-neural-networks/) builds the full architecture: how to stack layers, choose activations, initialize weights, and set up the forward pass. We'll apply it to the housing regression task and finally break through the $70k MAE barrier.
