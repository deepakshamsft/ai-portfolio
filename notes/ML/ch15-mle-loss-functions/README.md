...
> **Running theme:** Every loss function the platform has used — MSE for house price regression, binary cross-entropy for "high-value" classification — was chosen for a reason deeper than habit. Maximum Likelihood Estimation gives the principled derivation: the loss function is the log-likelihood of the data under a particular noise model. Change the noise model; change the loss.
