...
> **The story.** The **confusion matrix** has been with us since the 19th century in the contingency-table form **Karl Pearson** systematised in 1900. **ROC curves** were invented during the Second World War by US Army radar operators trying to distinguish enemy aircraft from flocks of birds — "Receiver Operating Characteristic" — and crossed over into psychology, then medicine, then ML. **AUC** as a metric was popularised by **James Hanley & Barbara McNeil** (Radiology, 1982). The **F1 score** is **Cornelis van Rijsbergen's** (1979) information-retrieval invention — a harmonic mean that punishes either precision or recall collapsing. **AUC-PR** entered the ML mainstream around 2006 (Davis & Goadrich) when the field finally admitted that AUC-ROC is misleading on heavily imbalanced datasets like fraud, click-through, and rare-disease detection. Every Kaggle leaderboard you have ever seen is one of these metrics in disguise.
>
> **Where you are in the curriculum.** The platform's high-value district classifier from [Ch.2](../ch02-logistic-regression/) is in production and a stakeholder asks: "How good is it?" The answer requires more than a single accuracy number. This chapter dissects the full classification and regression metrics toolkit so you can diagnose what your model actually does — which is exactly what every later chapter ([Ch.14](../ch14-unsupervised-metrics/), [Ch.19](../ch19-hyperparameter-tuning/), the AI-track evaluation notes) will assume you already know.
>
> **Notation in this chapter.** $TP,TN,FP,FN$ — confusion-matrix counts (true/false positives/negatives); accuracy $=\tfrac{TP+TN}{TP+TN+FP+FN}$; precision $P=\tfrac{TP}{TP+FP}$; recall $R=\tfrac{TP}{TP+FN}$ (also called *sensitivity* or *TPR*); $F_1=\tfrac{2PR}{P+R}$ — harmonic mean of precision and recall; $\text{FPR}=\tfrac{FP}{FP+TN}$; **ROC** — plot of TPR vs FPR across thresholds; **AUC** — area under the ROC curve; **PR curve** — precision vs recall; for regression: MAE $=\tfrac{1}{N}\sum|y-\hat{y}|$, RMSE $=\sqrt{\tfrac{1}{N}\sum(y-\hat{y})^2}$, $R^2$.
