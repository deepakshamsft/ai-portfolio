...
|---------|-------------|-----|
| **PR-AUC over ROC-AUC for imbalanced data** | [Topic 05 — Anomaly Detection](../../05_anomaly_detection/README.md) | Credit card fraud (0.17% positive rate) requires PR-AUC — ROC looks deceptively good |
| **Macro-averaged F1** | [Ch.5 — Hyperparameter Tuning](../ch05_hyperparameter_tuning) | FaceAI tuning uses `f1_macro` not accuracy to avoid the imbalance trap |
| **Hamming loss / multi-label metrics** | [Topic 03 — Neural Networks](../../03_neural_networks/README.md) | Multi-output neural network evaluation uses Hamming loss and per-label AUC |
| **StratifiedKFold** | [Ch.5 — Hyperparameter Tuning](../ch05_hyperparameter_tuning), every subsequent multi-class track | Required whenever class distribution is non-uniform |
