...
>
> **Decision flow animations** (see `notes/02-ai/ch11_advanced_agentic_patterns/gen_scripts/`):
...
def reflection_agent(user_input, max_iterations=3):
    """
    Reflection loop: Generate → Critique → Revise.
    
    Args:
        user_input: Customer order text
        max_iterations: Max refinement loops (prevent infinite iteration)
    
    Returns:
        final_response: Refined response after critique
        history: List of (draft, critique, revised) tuples
    """
    history = []
    draft = generate_draft(user_input)
    ...
    for i in range(max_iterations):
        ...
        # Check if critique suggests "good enough"
        if "no improvements needed" in critique.lower():
            return draft, history
        
        revised = revise_draft(draft, critique, user_input)
        history.append((draft, critique, revised))
        
        # Updated draft becomes input to next iteration
        draft = revised
    
    return draft, history

# Example usage
user_input = "gluten-free + dairy-free + extra cheese"
final_response, history = reflection_agent(user_input, max_iterations=3)

print("Final response:", final_response)
# Output: "For dairy-free, we use vegan mozzarella. Would you like extra
#          vegan mozzarella (+$2.50)? Total: $19.49."
```
...
### Code Skeleton: Multi-Agent Debate
...
def debate_agent(user_input, num_agents=2, num_rounds=2):
    """
    Multi-agent debate: Propose → Challenge → Defend → Vote.
    
    Args:
        user_input: Customer order or query
        num_agents: Number of agents (typically 2-3)
        num_rounds: Debate rounds (typically 1-2)
    ...
    # Round 1: Propose
    proposals = debate_propose(user_input, num_agents)
    history.append(("PROPOSE", proposals))
    
    for round_num in range(num_rounds):
        # Challenge
        challenges = debate_challenge(proposals)
        history.append((f"CHALLENGE_ROUND_{round_num+1}", challenges))
        
        # Defend
        defenses = debate_defend(proposals, challenges)
        history.append((f"DEFEND_ROUND_{round_num+1}", defenses))
        
        # Check for convergence (all agents agree)
        if all_agents_agree(defenses):
            break
    
    # Vote
    verdict = debate_vote(proposals, challenges, defenses)
    history.append(("VOTE", verdict))
    
    return verdict, history

# Example usage
user_input = "Customer has 20% coupon, $5 loyalty reward, BOGO promo. What's the total?"
verdict, history = debate_agent(user_input, num_agents=2, num_rounds=2)

print("Final verdict:", verdict)
# Output: "Customer should pay $18.98 (coupon + loyalty stacked)."
```
...
### Code Skeleton: Hierarchical Orchestration
...
def hierarchical_agent(user_input, max_workers=4):
    """
    Hierarchical orchestration: Plan → Execute → Verify.
    
    Args:
        user_input: Complex task description
        max_workers: Max parallel workers
    
    Returns:
        final_response: Verified result
        history: (plan, worker_results, verification)
    """
    # Step 1: Plan
    plan = hierarchical_plan(user_input)
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        futures = [executor.submit(worker_execute, subtask) for subtask in subtasks]
        worker_results = [f.result() for f in futures]
    
    # Step 3: Verify
    verification = hierarchical_verify(plan, worker_results)
    
    # Check if verification passed
    if "✅" in verification and "All constraints satisfied" in verification:
        final_response = synthesize_response(worker_results, verification)
    else:
        # Verification failed → retry or escalate
        final_response = handle_verification_failure(plan, worker_results, verification)
    
    return final_response, (plan, worker_results, verification)

# Example usage
user_input = """
15 pizzas, 3 delivery times (12pm, 1pm, 2pm), $200 budget, mix of veggie and meat.
"""
final_response, history = hierarchical_agent(user_input, max_workers=4)

print("Final response:", final_response)
# Output: "I can do 15 medium pizzas (5 vegetarian, 10 meat) with CATERING20
#          promo for $152.88 total (including delivery). Deliveries at 12pm,
#          1pm, 2pm. That's $47 under budget! Confirm?"
```
...
### Code Skeleton: Tool Selection with Fallback
...
class ToolWithFallback:
    """Tool with automatic fallback chain."""
    
    def __init__(self, name, cost, latency, accuracy, execute_fn):
        self.name = name
        self.cost = cost
    ...
    def execute(self, task):
        """Execute tool. Raises ToolFailure on error."""
        try:
            result = self.execute_fn(task)
            return {"success": True, "result": result, "tool": self.name}
        except Exception as e:
            raise ToolFailure(f"{self.name} failed: {e}")

def tool_selection_agent(task, context, strategy="cost_based"):
    """
    Select and execute tool with fallback chain.
    
    Args:
        task: Task description
        context: Runtime context (cache freshness, urgency, etc.)
        strategy: "rule_based", "cost_based", or "llm_based"
    
    Returns:
        result: Tool execution result
        history: Tool attempts and outcomes
    """
    # Define available tools
    tools = [
        ToolWithFallback("cache", 0.001, 0.1, 0.95, execute_cache),
        ToolWithFallback("database", 0.01, 0.5, 0.99, execute_database),
        ToolWithFallback("api", 0.05, 2.0, 1.00, execute_api),
    ]
    
    # Select tools based on strategy
    if strategy == "rule_based":
        selected_tools = rule_based_tool_selection(task, context)
        tools = [t for t in tools if t.name in selected_tools]
    
    elif strategy == "cost_based":
        tools = sorted(tools, key=lambda t: t.cost)  # Cheapest first
    
    elif strategy == "llm_based":
        selected_tool = llm_based_tool_selection(task, context, tools)
        tools = [selected_tool]  # Use only LLM-selected tool (no fallback)
    ...
    for tool in tools:
        try:
            result = tool.execute(task)
            history.append((tool.name, "SUCCESS", result))
            return result, history
        except ToolFailure as e:
            history.append((tool.name, "FAILURE", str(e)))
            logging.warning(f"{tool.name} failed: {e}")
            continue
    
    # All tools failed → escalate
    history.append(("human", "ESCALATED", "All tools failed"))
    return escalate_to_human(task), history

# Example usage
task = "Check inventory for pepperoni"
context = "Cache updated 5 min ago, customer is waiting"

result, history = tool_selection_agent(task, context, strategy="cost_based")

print("Result:", result)
# Output: {'success': True, 'result': {'pepperoni': 50}, 'tool': 'cache'}

print("History:", history)
# Output: [('cache', 'SUCCESS', {'pepperoni': 50})]
```
...
## 8 · Code Skeleton: Putting It All Together
...
class SmartPizzaBot:
    """PizzaBot v2.0 with all agentic patterns."""
    ...
    def _init_tools(self):
        """Initialize tools with fallback chains."""
        return {
            "inventory": [cache_tool, db_tool, api_tool],
            "pricing": [db_tool, api_tool],
            "menu": [cache_tool],
        }
    
    def route_to_pattern(self, user_input):
        """Route input to appropriate pattern."""
        # Classify input complexity
        if self._is_simple(user_input):
            return "single_pass"
        elif self._has_contradictions(user_input):
            return "reflection"
        elif self._is_high_stakes(user_input):
            return "debate"
        elif self._is_complex_multistep(user_input):
            return "hierarchical"
        else:
            return "single_pass"
    
    def _is_simple(self, user_input):
        """Check if input is simple (1-2 items, no edge cases)."""
        return (
            len(user_input.split()) < 15 and
            "coupon" not in user_input.lower() and
            "gluten" not in user_input.lower()
        )
    
    def _has_contradictions(self, user_input):
        """Check for contradictory keywords."""
        contradictions = [
            ("dairy-free", "cheese"),
            ("gluten-free", "regular crust"),
            ("vegan", "pepperoni"),
        ]
        return any(
            a in user_input.lower() and b in user_input.lower()
            for a, b in contradictions
        )
    
    def _is_high_stakes(self, user_input):
        """Check if decision is high-stakes (pricing disputes)."""
        return (
            user_input.lower().count("discount") +
            user_input.lower().count("coupon") +
            user_input.lower().count("promo")
        ) >= 2
    
    def _is_complex_multistep(self, user_input):
        """Check if task is complex (catering, multiple constraints)."""
        return (
            ("catering" in user_input.lower() or
             "company" in user_input.lower() or
             int(any(char.isdigit() and int(char) >= 10 for char in user_input))) and
            "budget" in user_input.lower()
        )
    
    def handle_order(self, user_input):
        """Main entry point: route to pattern and execute."""
        pattern = self.route_to_pattern(user_input)
        
        if pattern == "single_pass":
            return self.single_pass(user_input)
        elif pattern == "reflection":
            return self.reflection_agent(user_input)
        elif pattern == "debate":
            return self.debate_agent(user_input)
        elif pattern == "hierarchical":
            return self.hierarchical_agent(user_input)
    
    def single_pass(self, user_input):
        """Single-pass agent (Ch.1-10 baseline)."""
        prompt = f"Customer order: {user_input}. Generate order confirmation."
        response = self.llm.predict(prompt)
        return response
    
    def reflection_agent(self, user_input):
        """Reflection: Generate → Critique → Revise."""
        # Step 1: Draft
        draft = self.llm.predict(f"Draft order for: {user_input}")
        
        # Step 2: Critique
        critique = self.llm.predict(f"Critique this draft: {draft}")
        
        # Step 3: Revise
        if "no improvements" in critique.lower():
            return draft
        
        revised = self.llm.predict(
            f"Revise draft: {draft}\nCritique: {critique}"
        )
        return revised
    
    def debate_agent(self, user_input):
        """Debate: Propose → Challenge → Vote."""
        # Propose (2 agents)
        agent_1 = self.llm.predict(
            f"Customer-friendly pricing for: {user_input}"
        )
        agent_2 = self.llm.predict(
            f"Policy-strict pricing for: {user_input}"
        )
        
        # Vote
        judge = self.llm.predict(
            f"Pick best proposal:\n1. {agent_1}\n2. {agent_2}"
        )
        return judge
    
    def hierarchical_agent(self, user_input):
        """Hierarchical: Plan → Execute → Verify."""
        # Plan
        plan = self.llm.predict(f"Break down task: {user_input}")
        
        # Execute (4 workers)
        workers = [
            self.llm.predict(f"Subtask: Optimize sizes. Plan: {plan}"),
            self.llm.predict(f"Subtask: Split toppings. Plan: {plan}"),
            self.llm.predict(f"Subtask: Apply discounts. Plan: {plan}"),
            self.llm.predict(f"Subtask: Schedule delivery. Plan: {plan}"),
        ]
...
# Simple order → Single-pass
response = bot.handle_order("Large pepperoni pizza")
print(response)
# Output: "Large pepperoni ($14.99) + delivery ($3.00) = $17.99. Confirm?"

# Contradictory order → Reflection
response = bot.handle_order("Gluten-free, dairy-free, extra cheese")
print(response)
# Output: "For dairy-free, we use vegan mozzarella. Extra vegan cheese +$2.50?"

# Pricing dispute → Debate
response = bot.handle_order("I have a coupon, loyalty reward, and BOGO promo")
print(response)
# Output: "Your total is $18.98 (coupon + loyalty). That's $3.51 cheaper than BOGO."

# Catering order → Hierarchical
response = bot.handle_order("15 pizzas, 3 delivery times, $200 budget")
print(response)
# Output: "15 medium pizzas with CATERING20 = $152.88 (under budget by $47)."
```

---

## 9 · What Can Go Wrong: Common Traps Across All Patterns
...
## 10 · Progress Check: Can You Pick the Right Pattern?
...
## 11 · Bridge to Multi-Agent AI Track
...
**What's next:** The **Multi-Agent AI track** (`notes/03-multi_agent_ai/`) extends these patterns to persistent multi-agent systems:
...
**Next stop:** Multi-Agent AI track (`notes/03-multi_agent_ai/`) extends these patterns to persistent, coordinated, multi-agent systems.
