...
**The through-line in one paragraph.** Every chapter in this track is a rediscovery. Hewitt's Actor model (1973) and CMU's Hearsay-II blackboard (1975) became [SharedMemory](./SharedMemory/). KQML/FIPA-ACL (1990s) became [MCP](./MCP/) (Anthropic, Nov 2024) and [A2A](./A2A/) (Google, Apr 2025). Kafka-era pub/sub (LinkedIn 2011) became [EventDrivenAgents](./EventDrivenAgents/). ReAct (Yao et al., Oct 2022) and AutoGen (Microsoft, Sep 2023) made the LLM-agent loop — and frameworks like [LangGraph](./AgentFrameworks/) acknowledged it was a stateful graph, not a chain. What's actually new is that the *agent inside each node* is now an LLM — non-deterministic, expensive, and vulnerable to prompt injection (Greshake et al., 2023; OWASP LLM Top 10, 2023). That delta is why [TrustAndSandboxing](./TrustAndSandboxing/) exists and why [MessageFormats](./MessageFormats/) cares about token budgets.
