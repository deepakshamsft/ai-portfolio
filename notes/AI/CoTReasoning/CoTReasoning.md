...
> This note uses the PizzaBot system defined in [AIPrimer.md](../AIPrimer.md) as its running example. All tool names and queries below come from that system.
...
**Next chapter**: [RAG & Embeddings](../RAGAndEmbeddings/) connects the reasoning chain to real menu retrieval. CoT currently says "Action: retrieve_from_rag()" but doesn't execute it. Ch.4 makes that tool call real, grounding all answers in actual menu data → **<5% error rate, 18% conversion**.
