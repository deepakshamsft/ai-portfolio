...
>
> **Where you are in the curriculum.** The decision of *when* to fine-tune is more important than *how*. Most applications that reach for fine-tuning too early could have solved their problem with better [prompting](../PromptEngineering/) or [RAG](../RAGAndEmbeddings/) at a fraction of the cost. This document covers the decision framework first, then the efficient methods (LoRA, QLoRA, adapters) that make fine-tuning a Llama-class model on a laptop a realistic engineering option.
...
**Next chapter**: [Safety & Hallucination](../SafetyAndHallucination/) passes security audit → **approved for public launch, 100% allergen validation**.
