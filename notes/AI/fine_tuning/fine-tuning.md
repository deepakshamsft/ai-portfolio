...
>
> **Where you are in the curriculum.** The decision of *when* to fine-tune is more important than *how*. Most applications that reach for fine-tuning too early could have solved their problem with better [prompting](../prompt_engineering) or [RAG](../rag_and_embeddings) at a fraction of the cost. This document covers the decision framework first, then the efficient methods (LoRA, QLoRA, adapters) that make fine-tuning a Llama-class model on a laptop a realistic engineering option.
>
...
**Next chapter**: [Safety & Hallucination](../safety_and_hallucination) passes security audit → **approved for public launch, 100% allergen validation**.
